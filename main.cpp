...
inline bool isStoneFixed(Status& st, const int y, const int x, bool sl = false) {
    for(uint i = 0; i < 2; i++) {
...
void safeRoute(Input& in, Output& ou, vector<ScoredMove>& mv, Status& st, const uid_t id, const int y, const int x, const uint range, const uint beam, uint depth) {
    ...
    if(in.st[1].nin >= in.getCost(SKILL::OPMATEOR)) {
        ...
        if(neckList.size() < 3) {
            ...
            for(auto&& mm : mv1) {
                if(mm.sk == SKILL::MYTHUNDER) { mm.sk = SKILL::NONE; mm.score -= -INF / 4; }
            }
        }
    }
    ...
    while(true) {
        ...
        for(auto && m : mv1) {
            ...
            for(auto&& mm : mv2) {
                Point neck;
                if(cancelClosed(st, m.y, m.x, mm.mv, neck) != -1) { mm.score -= 20000; }
                Move m1 = m.mv;
...
bool greedNeck(Output& ou, Status& st, const int sy, const int sx, const int dy, const int dx) {
...
void findCritical(Output& ou, Input& in) {
    ...
    if(me.nin >= min(in.getCost(SKILL::OPMATEOR), in.getCost(SKILL::OPGHOST))) {
        for(auto& nj : op.ninja) {
            vector<ScoredMove> mv;
            ...
            if(mv.size() == 1) {
                ...
                if(in.getCost(SKILL::OPMATEOR) <= in.getCost(SKILL::OPGHOST)) {
                    ou.val1 = neck.y;
                    ou.val2 = neck.x;
    ...
    if(me.nin >= in.getCost(SKILL::OPMATEOR) + costmin 
        ...
        && in.getCost(SKILL::MYMATEOR) > in.getCost(SKILL::OPMATEOR)) {
        for(auto& nj : op.ninja) {
            DIR dir = DIR::NONE;
            for(auto& s : op.souls) {
    ...
    /*if(me.nin >= in.getCost(SKILL::TORNADO)) {
        for(auto&& ch : me.ninja) {
            if(in.getCost(SKILL::TORNADO) <= calcAroundDog(me, ch.y, ch.x) * 3) {
                for(uint i = 0; i < 8; i++) {
                    const auto& p = dir2p8(i);
